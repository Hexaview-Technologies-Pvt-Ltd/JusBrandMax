# Brand Visibility on Claude — Northwind

**Overall: 56.5/100** · model `claude-opus-4-8` · 10 prompts · 20 samples · standard mode · confidence: medium · 2026-06-27T00:00:00Z

## Executive summary

Northwind scores 56.5/100 for brand visibility on Claude (model claude-opus-4-8), measured across 10 prompts and 20 samples. It is mentioned in 50% of answers, with a 29% share of voice against its competitors. Visibility is 67% on direct "best <category>" questions but 25% on the problem-led questions buyers ask earlier in the journey. It is strongest on accuracy (67%) and weakest on share of voice (29%). The model makes 0 contradicted and 1 unsupported claim(s) about the brand. Top priority: Close the gap on "Most trusted fitness program options for fitness program?".

## Dimensions

| Dimension | Score |
|---|---|
| Presence (visibility) | 50% |
| Share of Voice | 29% |
| Prominence (first-mention rate) | 50% |
| Sentiment (net) | 1.00 |
| Accuracy | 67% (0 contradicted, 1 unsupported) |

## Intent breakdown

Where category demand forms vs. where you show up.

| Intent | Prompts | Visibility |
|---|---|---|
| Direct ("best <category>") | 6 | 67% |
| Indirect (problem / jobs-to-be-done) | 4 | 25% |

## Recommended actions

1. **Close the gap on "Most trusted fitness program options for fitness program?"** _(impact: high, effort: medium)_
2. **Close the gap on "Best fitness program recommended by professionals for fitness program?"** _(impact: high, effort: medium)_
3. **Close the gap on "What helps with fitness program?"** _(impact: high, effort: medium)_

## Competitor leaderboard

| Rank | Brand | Share | Mentions |
|---|---|---|---|
| 1 | Initech | 40% | 14 |
| 2 | Globex | 31% | 11 |
| 3 | **Northwind** | 29% | 10 |

## Gaps (competitors win, you're absent)

- **Most trusted fitness program options for fitness program?** → Globex, Initech
- **Best fitness program recommended by professionals for fitness program?** → Globex, Initech
- **What helps with fitness program?** → Globex, Initech
- **How do I choose the right fitness program?** → Globex, Initech
- **What questions should I ask before choosing a fitness program?** → Globex, Initech

---
_Generated by jusBrandMax — MIT, open-source brand visibility for Claude._
